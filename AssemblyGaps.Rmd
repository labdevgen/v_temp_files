...
graph <- function(gr){
  ...
  print(cbind(as.data.frame(sapply(split(lengthDF, lengthDF$type), 
                                   function(x) summary(x$length))), 
              do.call(rbind, Map(data.frame, all.types= summary(lengthDF$length)))))
  
  g1 <- ggplot(lengthDF, aes(x=length)) + geom_histogram(colour='white') +
    scale_y_continuous(trans='sqrt', name='sqrt(count)') + 
    scale_x_continuous(trans='sqrt', name='sqrt(length)') 
  
  g2 <- ggplot(lengthDF, aes(x=length)) + geom_histogram(colour='white') + 
